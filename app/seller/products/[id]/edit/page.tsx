  ...
  const fetchProduct = async () => {
    try {
      ...
      if (result.success) {
        ...
        setErrors({ fetch: result.message || 'Failed to fetch product' });
      }
    } catch (error) {
      console.error('Error fetching product:', error);
      setErrors({ fetch: 'An error occurred while fetching the product' });
    } finally {
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="flex items-center px-4 py-2 border-2 border-dashed border-gray-300 rounded-lg hover:border-green-500 hover:bg-green-50 transition-colors w-full justify-center"
            >
              <Upload className="w-5 h-5 mr-2 text-gray-500" />
